...
const WhatsAppFloat = ({ whatsappNumber }: WhatsAppFloatProps) => {
  ...
  // Format WhatsApp number for URL (remove spaces, dashes, parentheses)
  const formatWhatsAppNumber = (number: string) => {
    return number.replace(/[\s\-\(\)]/g, '');
  };

  const whatsappUrl = `https://wa.me/${formatWhatsAppNumber(whatsappNumber)}?text=Hello! I'm interested in your conference services.`;
    ...
    >
      ...
      <button
        ...
        className={`
          ...
          ${isHovered ? 'w-auto px-4 py-3' : 'w-14 h-14'}
        `}
      ...
      >
        {/* WhatsApp Icon */}
        <svg
          className={`transition-all duration-300 ${isHovered ? 'w-6 h-6 mr-2' : 'w-7 h-7'}`}
          fill="currentColor"
          viewBox="0 0 24 24"
      ...
      <div
        className={`
          absolute bottom-full right-0 mb-2 px-3 py-2 bg-gray-800 text-white text-sm rounded-lg
...
// Server component to fetch WhatsApp number from Sanity
export default async function WhatsAppFloatWrapper() {
  ...
  return <WhatsAppFloat whatsappNumber={whatsappNumber} />;
}
